...
#from probe.crawler.configs.auto import caam as config


class CaamSpider(IndustryInfoSpider):
    """
    @summary: 中国汽车工业协会爬虫
    ...
    def _get_result(self, response):    # config在下面定义
        ...
        """
        rel = []
        config = response.meta['conf']  # config
        try: 
    ...
    def _finish_item(self, item, response):
        ...
        try:
            # 标题title     
            title = ' '.join(re.split(" ", response.xpath("//title/text()").extract()[0])[:-2]).strip()
            # 判断和处理【关键字】标题特殊情况
            ...
            # 源网站source_website
            source_website = response.xpath("//div[@class='timecont']/ul/li[last()]/text()").extract()
            if len(source_website):
                item['source_website'] = source_website[0]
            # 资讯内容content
