...
#from probe.crawler.configs.auto import auto360 as config 


class Auto360Spider(IndustryInfoSpider):
    """
    @summary: 中国汽车资讯中心网爬虫
    ...
    def _get_result(self, response):       # config在下面定义
        ...
        try:
            ...
            for ul in results:      
                ...
                # 发布时间publish_time 
                publish_time = ul.xpath("./span[@class='date']/text()").extract()[0]
                if re.match("\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d", publish_time):
    ...
    def _finish_item(self, item, response):
        ...
        try:
            ...
            # 发布时间publish_time
            if item['publish_time'].hour == 0 and item['publish_time'].minute == 0 and item['publish_time'].second == 0:
                publish_time = response.xpath("//span[@class='article-published']/text()").extract()[0]
                item['publish_time'] = datetime.strptime(publish_time, "%Y-%m-%d %H:%M")
            # 源网站source_website和源链接source_url
            source_website = response.xpath("//span[@class='article-source']/text()").extract()[0]
            if u"作者" in source_website:
                item['author'] = source_website.split(u'：')[-1].strip()
                item['source_website'] =re.split(u':|作者', source_website)[1].strip()
            else:
                item['source_website'] =source_website.split(':')[-1].strip()
            item['source_url'] = u""
            # 内容content 
            content = response.xpath("//div[@class='content']").extract()
            if len(content) <= 0:
                content = response.xpath("//div[@id='ctrlfscont']").extract()
            item['content'] = HtmlFormatter.format_content(content[0])
            # 资讯配图url(pipeline处理)     
            item['image_urls'] = []
            for imgurl in response.xpath("//div[@class='content']//img/@src").extract():
                img_url = re.sub("\s","",imgurl)
                m = re.match("[\s\S]+(\.jpg|\.JPG|=jpg|\.png|\.PNG|=png|\.jpeg|\.JPEG|=jpeg|\.bmp|\.BMP|=bmp|\.gif|=gif|\.GIF)", img_url)
                if m:
                    img_url = m.group()
                else:
                    logger.warning("Warning##{0}##_finish_item##img_url: {1}".format(self.name, img_url))
                if re.match("http.*", img_url):
                    img_url = "http" + img_url.split("http")[-1]
                    item['image_urls'].append(img_url)
                else:
                    item['image_urls'].append("http://www.autochina360.com"+img_url)
        except Exception, e:
