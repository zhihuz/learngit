...
class CpnnSpider(IndustryInfoSpider):
    """
    @summary: 智能机器人网站资讯爬虫
    @mark:继承抽象基类IndexSpider
    """
    name = "energy.cpnn"                     # 爬虫名称
    ...
    def _get_result(self, response):
        ...
        try:
            results = response.xpath("//div[@class='cpnn-content-left-list']/ul/li")
            if not results:
            ...
            for ul in results:
                ...
                # 爬取链接
                conf = config['url']
                crawl_url = ul.xpath("./h1/a/@href").extract()[0]
                item["crawl_url"] = crawl_url.replace("./",conf[:conf.rfind("/")+1])
                # 发布时间publish_time
                publish_time = "20" + ul.xpath("./h1/div[@class='cpnn-list-date']/text()").extract()[0]
    ...
    def _next_result_page(self, response):
        ...
        try:
            curUrl = response.url
            pageJs = response.xpath("//div[@class='cpnn-content-left-list']/div[@class='ep-pages']").extract()[0]
            maxPage = int(re.findall("countPage = ([0-9]+)",pageJs)[0])
            attr = re.findall("_([0-9]+).htm",curUrl)
            curPage = int(attr[0]) if attr else 0
            if maxPage >1 and curUrl.endswith("default.htm"):
                return curUrl[:-4]+"_1.htm"
            elif curPage < maxPage -1:
                return re.sub("[0-9]+",str(curPage + 1),curUrl)
        except Exception, e:
            traceback.print_exc()
    ...
    def _finish_item(self, item, response):
        ...
        try:
            # 标题title
            title = re.split("丨", response.xpath("//div[@class='hdtitle']/h1/text()").extract()[0])[0].strip()
            # 判断和处理【关键字】标题特殊情况
            ...
            item['keyword'] = ','.join(list(set(item['keyword'].split(',')))) # set用于去掉重复关键字
            # 摘要
            item['digest'] = u''
            item["source_url"] = u""
            item['source_website'] = u"中电新闻网"
            # 提取content
            content = response.xpath("//div[@class='cpnn-con-zhenwen']").extract()[0]
