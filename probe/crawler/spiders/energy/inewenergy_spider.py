...
class InewenergySpider(IndustryInfoSpider):
    """
    @summary: 智能机器人网站资讯爬虫
    @mark:继承抽象基类IndexSpider
    """
    name = "energy.inewenergy"                     # 爬虫名称
    ...
    def _get_result(self, response):
        ...
        try:
            results = response.xpath("//ul[@class='am-list']/li")
            if not results:
            ...
            for ul in results:
                ...
                # 发布时间publish_time
                publish_time = ul.xpath("./div/div/text()[1]").extract()[0].strip()
    ...
    def _next_result_page(self, response):
        ...
        try:
            curUrl = response.url
            # 当前所抓取的页码
            page = int(re.findall("([0-9]*).html",curUrl)[0])
            # 从目录页中获取的最后一页的页码
            maxpage = int(response.xpath("//span[@class='pageinfo']/strong[1]/text()").extract()[0])
            maxpage = 2
            if page < maxpage:
                page += 1
            else:
                ...
                return
            nextUrl = re.sub("[0-9]*(.html)",str(page)+r"\g<1>",curUrl)
            return nextUrl
        except Exception, e:
            traceback.print_exc()
    ...
    def _finish_item(self, item, response):
        ...
        try:
            ...
            item['digest'] = u''
            item["source_url"] = u""
            item['source_website'] = response.xpath("//h4[@class='am-article-meta blog-meta']/text()[1]").extract()[0]
            # 提取content
            content = response.xpath("//div[@class='am-g blog-content']//p").extract()
            item['content'] = HtmlFormatter.format_content("\n".join(content))
            # 资讯配图url(pipeline处理)
            item['image_urls'] = []
            for img_url in response.xpath("//div[@class='am-g blog-content']//img/@src").extract():
                if img_url.startswith("http"): #以http打头的链接是淘宝店铺二维码,应过滤
                    item['image_urls'].append(img_url)
                else:
                    item['image_urls'].append("http://www.inewenergy.com"+img_url)
        except Exception, e:
            traceback.print_exc()
