...
class GlinfoSpider(IndustryInfoSpider):
    """
    @summary: 大宗商品网爬虫
    @mark: 继承抽象基类IndustryInfoSpider
    """
    name = "steel.glinfo"                     # 爬虫名称
    ...
    def _get_result(self, response):       # config在下面定义
        ...
        try:
            results = response.xpath("//ul[@class='nlist']/li")
            if not results:
    ...
    def _next_result_page(self, response):
        ...
        """
        # 当前所抓取的页码
        # 首页url以index.html结尾，第二页为index_1.html
        attr = re.split("re.split('-----|.html',a)", response.url)
        # 当前所抓取的页码
        page = int(attr[-2])
        # 从目录页中获取的最后一页的页码
        pages = response.xpath("//div/div[@class='page']/a[last()-1]/text()").extract()[0]
        maxpage = int(pages)
        if page < maxpage:
            page += 1
            attr[-2] = str(page)
    ...
    def _finish_item(self, item, response):
        ...
        try:
            # 标题title
            title = re.split("_", response.xpath("//div[@id='articleContent']/h1/text()").extract()[0])[0].strip()
            # 判断和处理【关键字】标题特殊情况
            ...
            source_website = response.xpath("//div[@id='articleContent']/div[@class='info']/text()").extract()[0].encode("utf-8").strip()
            item['source_website'] = re.split(' ',source_website)[-1]
            item['source_url'] = u""
            # 内容content，获取的content为段落列表，需要整合到一个字符串里
            content = response.xpath("//div[@id='articleContent']/div[@id='text']/p").extract()
            contents = ''
            for i in content:
                contents = contents + str(i) +'\n'
            item['content'] = HtmlFormatter.format_content(contents)
            # 资讯配图url(pipeline处理)
            item['image_urls'] = []
            for img_url in response.xpath("//div[@id='text']/p/img/@src").extract():
                if re.match("http.*", img_url):
                    item['image_urls'].append(img_url)
                else:
                    item['image_urls'] = 'http://img02.mysteelcdn.com/wz/uploaded/glinfo/' + img_url
        except Exception, e:
