...
import sys
sys.path.append("/home/sybil/project")
...
class ChinatsiSpider(IndustryInfoSpider):
    """
    @summary: 爬虫
    @mark: 继承抽象基类IndustryInfoSpider
    """
    name = "steel.chinatsi"                     # 爬虫名称

    #用于index页面翻页用，这里进行初始化
    now = datetime.now()
    year = now.year
    month = now.month
    ...
    def _get_result(self, response):       # config在下面定义
        ...
        try:
            logger.warning("now is dig:"+response.url)
            results = response.xpath('//div[@class="box_c"]/ul/li')
            if not results:
    ...
    def _next_result_page(self, response):
        ...
        @return: 下一页目录的URL或空值
        """
        #index页面对应的url形如http://news.chinatsi.com/newslist.aspx?typeId=31&RQ=2016-4，RQ=当前日期

        #获取当前index页面日期
        cur_date = re.findall(r'\d+', response.xpath("//div/h1/div/text()").extract()[0])
        self.year = int(cur_date[0])
        self.month = int(cur_date[1])
        #生成下一个index页面对应的url里的日期，即RQ的值
        next_month = self.month -1 or 12
        next_year = self.year -1 if next_month == 12 else self.year
        next_date = str(next_year) + "-" + str(next_month)
        #拼接下一页url
        return response.url + next_date if response.url[-1] == "=" else response.url.split("&")[0] + "&RQ=" + next_date
    ...
    def _finish_item(self, item, response):
        ...
        try:
            # 标题title
            title = re.split("-", response.xpath("//span[@id='lblTitle']/text()").extract()[0].strip())[0]
            # 判断和处理【关键字】标题特殊情况
            ...
            item['source_website'] = response.xpath("//span[@id='lblTime']/text()").extract()[0].split(" ")[1][4:]
            item['source_url'] = u""
            # 内容content
            content = response.xpath("//div[@id='detail']/span[@id='lblContent']").extract()[0]
            ...
            #     else:
            #         url_list = item['crawl_url'].split('/')[:-1]
            #         url_list.append(img_url[2:])
            #         item['image_urls'] = '/'.join(url_list)
        except Exception, e:
