...
class CsteelnewsSpider(IndustryInfoSpider):
    """
    @summary: 中国钢铁新闻网爬虫
    @mark: 继承抽象基类IndustryInfoSpider
    """
    name = "steel.csteelnews"                     # 爬虫名称
    ...
    def _get_result(self, response):       # config在下面定义
        ...
        try:
            results = response.xpath("//ul[@class='lieb']/li")
            if not results:
            ...
            for ul in results:
                ...
                index_url = response.url.split('/')
                index_url[-1] = ul.xpath("./a/@href").extract()[0][2:]
                item['crawl_url'] = '/'.join(index_url)
                # 发布时间publish_time
    ...
    def _next_result_page(self, response):
        ...
        if response.url.endswith("index.html"):
            return response.url.replace("index","index_1")
        attr = re.split("_|.html", response.url)
        page = int(attr[-2])
        # 观察得所有栏目均为22页,第二页index为1，所以最后一页对应的index应为21
        maxpage = 21
        if page < maxpage:
            page += 1
            attr[-2] = str(page)
    ...
    def _finish_item(self, item, response):
        ...
        try:
            # 标题title
            title = re.split("-", response.xpath("//div[@class='zwcon']/h1//text()").extract()[0].strip())[0]
            # 判断和处理【关键字】标题特殊情况
            ...
            item['source_website'] = response.xpath("//div[@style='margin:10px 0px; text-align:right']/text()").extract()[0].split('：')[-1].strip()
            item['source_url'] = u""
            # 内容content
            content = response.xpath("//div[@class='zw']").extract()
            item['content'] = HtmlFormatter.format_content(content[0])
            # 资讯配图url(pipeline处理)
            item['image_urls'] = []
            for img_url in response.xpath("//div[@class='zw']//img/@src").extract():
